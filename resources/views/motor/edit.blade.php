...
@section('title', 'Edit Jenis Motor - Sistem Pakar Motor')
    ...
    <header class="bg-gray-900/80 backdrop-blur-sm border-b border-gray-700 sticky top-0 z-10">
        <div class="px-8 py-6">
            <h1 class="text-3xl font-bold text-white">
                <i class="fas fa-edit text-orange-400 mr-3"></i>Edit Jenis Motor
            </h1>
            <p class="text-gray-400 mt-1">Edit data motor: {{ $motor->nama_motor }}</p>
        </div>
    </header>
        ...
        <div class="max-w-2xl">
            <form action="{{ route('motor.update', $motor->id) }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="space-y-6">
                    ...
                    @if($motor->gambar)
                        <div class="mb-4">
                            <label class="block text-white font-medium mb-2">Gambar Saat Ini</label>
                            <div class="flex items-center gap-4">
                                <img src="{{ asset('uploads/motor/' . $motor->gambar) }}" alt="{{ $motor->nama_motor }}" class="w-32 h-32 object-cover rounded-lg border border-gray-700">
                    ...
                    <div>
                        ...
                        <select name="transmisi" required
                            ...
                            <option value="matic" {{ old('transmisi', $motor->transmisi) == 'matic' ? 'selected' : '' }}>Matic</option>
                            <option value="manual" {{ old('transmisi', $motor->transmisi) == 'manual' ? 'selected' : '' }}>Manual</option>
                        </select>
                        @error('transmisi')
                            <p class="mt-2 text-red-400 text-sm">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <textarea name="deskripsi" rows="3"
                                  ...
                                  class="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-3 text-white
                                         placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-orange-500
                                         focus:border-orange-500 transition resize-none">{{ old('deskripsi', $motor->deskripsi) }}</textarea>
                        @error('deskripsi')
                            <p class="mt-2 text-red-400 text-sm">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        <label class="block text-white font-medium mb-2">
                            <i class="fas fa-image text-orange-400 mr-2"></i>Ganti Gambar Motor
                        </label>
                        ...
                        <p class="text-gray-400 text-xs mt-2">Kosongkan jika tidak ingin mengubah gambar</p>
                        @error('gambar')
                            <p class="mt-2 text-red-400 text-sm">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" name="is_active" id="is_active" value="1" {{ $motor->is_active ? 'checked' : '' }}
