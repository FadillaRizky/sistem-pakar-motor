...
<head>
    ...
    <title>{{ config('app.name', 'Sistem Pakar Motor') }}</title>
...
<div class="min-h-screen flex bg-slate-950 text-slate-100">
    {{-- SIDEBAR USER: hanya Dashboard + Logout --}}
        ...
        <nav class="flex-1 px-3 py-4 space-y-1 text-sm">
            <a href="{{ route('dashboard') }}"
               class="flex items-center px-3 py-2 rounded-lg
                      {{ request()->routeIs('dashboard') ? 'bg-slate-800 text-white' : 'text-slate-300 hover:bg-slate-800/70' }}">
                Dashboard
            </a>
        </nav>
